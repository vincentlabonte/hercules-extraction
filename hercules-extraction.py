...
DEFAULT_CONFIG_FILE = DIR_PATH / 'utils' / 'default_config.yml'
SCHEMA_CONFIG_FILE = DIR_PATH / 'utils' / 'schema.yml'
...
def main():
    ...
    if translation_config is not None:
        entities = translate_back_triples(entities, translation_config)

    export_config = config.get('export')
    if export_config is not None:
        exported_rdf = export_entities(entities, export_config)

    if args.out is not None:
        out_path = Path(args.out)
        out_path.write_text(exported_rdf, encoding='utf-8')
    else:
        print(exported_rdf)
...
def translate_text(text, translation_config):
    ...
    translated_text = translator.translate(
        text, language_text, language_extraction)
    return translated_text

def translate_back_triples(entities, translation_config):
    translator_name = translation_config.get('implementation')
    language_config = translation_config.get('language')
    language_text = language_config.get('text')
    language_extraction = language_config.get('extraction')
...
def resolve_coreference(text, entities, coreference_config):
    ...
    coreference_resolver_type = registered_coreference_resolver.get(
        coreference_resolver_name)
    if coreference_resolver_type == coreference.StanfordCoreferenceResolver:
        ...
        coreference_resolver = coreference.StanfordCoreferenceResolver(
            **stanford_args)
    else:
...
def export_entities(entities, export_config):
    ...
    rdf_export = exporter.export(
        entities, entity_namespace, ontology_namespace, export_language)
    return rdf_export
